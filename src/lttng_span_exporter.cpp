...
#include "lttng_span_exporter_lttng_tracepoint.h"
...
opentelemetry::v1::exporter::trace::LttngSpanExporter::Export(
	...
	for (auto &recordable_span : spans) {
		...
		lttng_ust_tracepoint(otel_with_lttng, otel_span,
		                     span_serialized.c_str());
		// DEBUG
		// std::string span_debug = span->span().DebugString();
		// lttng_ust_tracepoint(otel_with_lttng, otel_debug_span,
		//                      span_debug.c_str());
	}
